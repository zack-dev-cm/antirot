#!/usr/bin/env python3
"""Audit an unpacked ClawHub/Codex skill before installation."""
...
PUBLISH_SURFACE_RULES = (
    Rule(
        "platform-outreach-surface",
        "WARNING",
        re.compile(
            r"\b(linkedin|discord|telegram|whatsapp|slack|instagram|facebook|"
            r"tiktok|youtube studio|gmail|x/twitter|twitter)\b",
            re.IGNORECASE,
        ),
        "Named outreach or social platform wording requires ClawHub publish-surface review.",
    ),
    Rule(
        "logged-in-browser-control",
        "WARNING",
        re.compile(
            r"\b(openclaw|computer use|browser automation|logged-in browser|"
            r"browser profile|session profile|cookie-backed|authenticated browser)\b",
            re.IGNORECASE,
        ),
        "Logged-in browser or desktop-control wording requires ClawHub publish-surface review.",
    ),
    Rule(
        "automated-message-delivery",
        "WARNING",
        re.compile(
            r"\b(auto(?:mate[sd]?|matic)?[- ]?(?:send|dm|message|reply|post)|"
            r"mass[- ]?(?:dm|message)|selfbot|scrap(?:e|ing)|"
            r"send(?:ing)?\s+(?:a\s+|the\s+)?(?:dm|message|reply|comment)|"
            r"post(?:ing)?\s+(?:a\s+|the\s+)?(?:message|reply|comment))\b",
            re.IGNORECASE,
        ),
        "Automated delivery, selfbot, scraping, or direct-message wording requires publish-surface review.",
    ),
)
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Audit an unpacked ClawHub/Codex skill before installation."
    )
    parser.add_argument("skill_path", help="Path to the unpacked skill directory")
    ...
    if args.json:
        ...
        print(render_text_report(root, verdict, findings))
...
def audit_skill(root: Path) -> list[Finding]:
    ...
    if not root.exists():
        return [
            Finding("BLOCK", "missing-skill", str(root), None, "Skill path does not exist.")
        ]
    if not root.is_dir():
        return [
            Finding("BLOCK", "not-directory", str(root), None, "Skill path is not a directory.")
        ]

    skill_md = root / "SKILL.md"
    if not skill_md.exists():
        findings.append(
            Finding("BLOCK", "missing-skill-md", "SKILL.md", None, "Missing required SKILL.md.")
        )
    else:
...
def audit_file_metadata(root: Path, path: Path, rel_path: str) -> list[Finding]:
    ...
    if suffix in {".key", ".pem", ".p12", ".pfx"} or name in {"id_rsa", "id_dsa", "id_ecdsa", "id_ed25519"}:
        findings.append(
            Finding("BLOCK", "credential-file", rel_path, None, "Credential or key-like file is present.")
        )

    if suffix in ARCHIVE_SUFFIXES:
        findings.append(
            Finding("BLOCK", "archive-payload", rel_path, None, "Archive payload must be unpacked and audited before install.")
        )
...
def audit_binary_file(path: Path, rel_path: str) -> list[Finding]:
    ...
    if os.access(path, os.X_OK):
        return [
            Finding("BLOCK", "binary-executable", rel_path, None, "Executable binary or unreadable executable is present.")
        ]
    if suffix in KNOWN_BINARY_ASSET_SUFFIXES:
        return [
            Finding("WARNING", "binary-asset", rel_path, None, "Binary asset requires provenance review.")
        ]
    if suffix not in TEXT_SUFFIXES:
        return [
            Finding("WARNING", "unknown-binary", rel_path, None, "Unreadable or unknown binary file requires review.")
        ]
    return []
...
def audit_skill_md(root: Path, text: str) -> list[Finding]:
    ...
    if not lines or lines[0].strip() != "---":
        return [
            Finding("BLOCK", "missing-frontmatter", "SKILL.md", 1, "SKILL.md must start with YAML frontmatter.")
        ]
    ...
    if end is None:
        return [
            Finding("BLOCK", "unterminated-frontmatter", "SKILL.md", 1, "SKILL.md frontmatter is not closed.")
        ]
    ...
    for field in sorted(missing):
        findings.append(
            Finding("BLOCK", "missing-frontmatter-field", "SKILL.md", None, f"Missing frontmatter field: {field}.")
        )

    extra = sorted(set(fields) - {"name", "description"})
    for field in extra:
        findings.append(
            Finding("WARNING", "extra-frontmatter-field", "SKILL.md", None, f"Unexpected frontmatter field: {field}.")
        )

    skill_name = fields.get("name", "")
    if skill_name and skill_name != root.name:
        findings.append(
            Finding("WARNING", "name-folder-mismatch", "SKILL.md", None, "Skill name does not match the folder name.")
        )
    if any("TODO" in value or "Complete" in value for value in fields.values()):
        findings.append(
            Finding("BLOCK", "template-frontmatter", "SKILL.md", None, "Template placeholder remains in frontmatter.")
        )
    if len(fields.get("description", "")) < 80:
        findings.append(
            Finding("WARNING", "thin-description", "SKILL.md", None, "Description is too thin to support safe triggering.")
        )
...
def audit_text_file(rel_path: str, text: str) -> list[Finding]:
    ...
    if "TODO:" in text or "[TODO" in text:
        findings.append(
            Finding("WARNING", "template-marker", rel_path, None, "Template TODO marker remains in the skill.")
        )

    for line_number, line in enumerate(text.splitlines(), start=1):
        for rule in (*SECRET_RULES, *BLOCK_RULES, *WARNING_RULES, *PUBLISH_SURFACE_RULES):
            if rule.pattern.search(line):
                if rule in SECRET_RULES and looks_like_placeholder(line):
                    continue
                findings.append(
                    Finding(rule.severity, rule.code, rel_path, line_number, rule.message)
                )
    return findings
...
def audit_dependency_file(rel_path: str, text: str) -> list[Finding]:
    ...
    if path_name == "requirements.txt":
        ...
        findings.append(
            Finding("WARNING", "dependency-manifest", rel_path, None, "Dependency manifest requires manual review.")
        )
    return findings
...
def render_text_report(root: Path, verdict: str, findings: list[Finding]) -> str:
    lines = [
        f"ClawHub skill security audit for {root}",
